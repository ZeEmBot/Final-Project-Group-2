...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['usecase'])) {
    if ($usecase === 'city' && !empty($cityName)) {
        ...
        $query = "SELECT city_name, latitude, longitude FROM geolocation WHERE city_name = '$cityNameEscaped'";
...
<body>
    <div class="container">
        ...
        <ul>
            ...
            <li><b>Usecase 2 :</b> Cari data kota yang memiliki temperatur suhu yang sama.</li>
        </ul>
        ...
        <form method="GET" action="">
            ...
            <select id="usecase" name="usecase">
                ...
                <option value="temperature" <?= $usecase === 'temperature' ? 'selected' : '' ?>>Cari Temperatur suhu yang
                    sama</option>
            </select>

            <div id="city-input" style="display: <?= $usecase === 'city' ? 'block' : 'none' ?>;">
                <label for="city_name">Nama Kota:</label>
                <input type="text" id="city_name" name="city_name" placeholder="Contoh: Jakarta"
            ...
            <div id="temperature-input" style="display: <?= $usecase === 'temperature' ? 'block' : 'none' ?>;">
                ...
                <input type="number" id="temperature" name="temperature" placeholder="Contoh: 30"
                    value="<?= htmlspecialchars($temperature) ?>">
            </div>
        ...
        <div id="output">
            <?php if ($usecase === 'city' && !empty($data)) {
                echo '<h2>Hasil Pencarian Geolokasi:</h2>';
                echo '<table>';
                echo '<thead><tr><th>Nama Kota</th><th>Latitude</th><th>Longitude</th></tr></thead>';
                echo '<tbody>';
                echo '<tr>';
                echo '<td>' . htmlspecialchars($data['city_name']) . '</td>';
                echo '<td>' . htmlspecialchars($data['latitude']) . '</td>';
                echo '<td>' . htmlspecialchars($data['longitude']) . '</td>';
                echo '</tr>';
                echo '</tbody>';
                echo '</table>';
            } elseif ($usecase === 'temperature' && isset($_GET['temperature'])) {
                    ...
                    echo '<script>
                            function showDetails() {
                                ...
                                cityNames.forEach(city => {
                                    ...
                                    // fetch(`http://bmkg.com/RealTAtst/weather/api.php?city_name=${encodeURIComponent(city)}`)
                                        .then(response => {
